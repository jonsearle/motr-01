...
function MotorHqLoginContent() {
  const router = useRouter();
  const searchParams = useSearchParams();
  const next = searchParams.get("next") || "/motorhq";
  ...
  async function onSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    try {
      const response = await fetch("/api/motorhq/auth", {
        method: "POST",
  ...
  return (
    <main className="min-h-screen bg-[#FBFCFE] text-[#1C2330]">
      <div className="mx-auto w-full max-w-md px-6 pb-20 pt-12">
        <h1 className="text-[30px] font-semibold tracking-[-0.02em]">MotorHQ</h1>
        ...
        <form onSubmit={onSubmit} className="mt-6 space-y-4">
          ...
          >
            {submitting ? "Unlocking..." : "Unlock MotorHQ"}
          </button>
        </form>
      </div>
    </main>
...
export default function MotorHqLoginPage() {
  return (
